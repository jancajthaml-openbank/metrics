...
def dockerOptions() {
    ...
    options += "packaging/docker/Dockerfile"
    return options
}
...
pipeline {
    ...
    stages {
        ...
        stage('Setup') {
            steps {
                script {
                    ...
                    env.PROJECT_NAME = "openbank metrics"
                    env.PROJECT_DESCRIPTION = "OpenBanking statsd metrics daemon"
        ...
        stage('Package Docker') {
            steps {
                script {
                    DOCKER_IMAGE = docker.build("${env.ARTIFACTORY_DOCKER_REGISTRY}/docker-local/openbank/metrics:${env.VERSION}", dockerOptions())
                }
            }
        }
            ...
            steps {
                script {
                    ...
                    artifactory.upload spec: """
                    {
                        "files": [
                            {
                                "pattern": "${env.WORKSPACE}/packaging/bin/metrics_(*)_(*).deb",
                                "target": "generic-local/openbank/metrics/{1}/linux/{2}/metrics.deb",
                                "recursive": "false"
                            }
                        ]
                    }
